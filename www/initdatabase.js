...
function initdb(){
    ...
    db.transaction(
         function(tr){
             tr.executeSql('CREATE TABLE IF NOT EXISTS TestTable (title unique,info,mapx,mapy,opentime INTEGER,endtime INTEGER,time,season,slope,imageurl)');
              
              for(var i = 0;i<datalen;i++){
               tr.executeSql('INSERT INTO TestTable VALUES(?,?,?,?,?,?,?,?,?,?)',[titles[i] ,informations[i],mapxs[i],mapys[i],opentimes[i],endtimes[i],times[i],seasons[i],slopes[i],imageurls[i]]);
